...
def main() -> None:
    ...
    if args.best_param is not None and str(args.best_param).strip() != "":
        ...
        # Accept either:
        #  - {"best_params": {...}}
        #  - {...} (direct dict of params)
        bp = obj.get("best_params", obj)

        # --- hidden sizes from h1/h2 ---
        h1 = int(bp["h1"])
        h2 = int(bp.get("h2", 0))
    ...
    # If best_param is NOT provided, require hidden_sizes explicitly (backward compatible)
    ...
    # slicing day value (based on unique days in masked data)
    ...
    # IMPORTANT: original code overwrote day_use with the 3rd return (which becomes group id).
    # To keep logic identical, we do NOT change how it affects training/eval here.
    # (Per-day accuracy later is computed on the val subset using day_use[va_idx] from the ORIGINAL vector.)
    # Therefore, we keep day_use separate and only index it with va_idx before lagging.
    # The original script computed day_val = day_use[va_idx] BEFORE lagging (via va_idx from N=X_use.shape[0]).
    # To keep identical behavior, we must keep day_use aligned with X_use at split time.
    # Since the original script performed split after lagging, day_use must correspond to lagged samples.
    # In the original file, day_use became "group_eff" (trial/day) at that point.
    # We keep exactly that behavior by reconstructing day_use the same way:
    if args.n_lags > 0:
        # Recompute day_use to match original behavior: it becomes the lag-group effective vector.
        # group_eff returned by make_lagged_features is the group id, not day.
        # This is exactly how the original script behaved.
    ...
    # label dist (within used/lagged data)
    ...
    # -------------------------
    # (6) Seeds loop (unchanged: default single seed 0)
    # -------------------------
    seeds = list(range(10))  # identical behavior to the original script
    ...
    for seed in seeds:
        ...
        # shuffle split within used data
        perm = np.random.permutation(N)
        ...
        # IMPORTANT: original script uses day_val from "day_use[va_idx]" AFTER day_use is overwritten.
        ...
        # per-day accuracy (computed on val subset only; unchanged logic)
        ...
        # -------------------------
        # (7) Save artifacts (filenames now include hidden structure tag)
        # -------------------------
    ...
    # -------------------------
    # (8) Aggregate mean/std across seeds (unchanged logic)
    # -------------------------
